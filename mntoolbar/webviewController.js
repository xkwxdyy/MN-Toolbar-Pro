...
try {
  ...
  setColor: function (button) {
    ...
    let fillIndex = -1
    let description = toolbarConfig.actions["color"+color].description
  ...
  try {
    ...
    switch (des.action) {
      ...
      case "addImageComment":
        ...
        self.compression = des.compression ?? true
        self.currentNoteId = focusNote.noteId
        switch (source) {
          case "camera":
        ...
        // self.presentViewControllerAnimatedCompletion(self.imagePickerController,true,undefined)
        // 展示图片选择器
      ...
      case "focus":
        switch (des.target) {
          case "doc":
            // MNUtil.showHUD("message"+focusNote.noteId)
            // MNUtil.copy(focusNote.noteId)
            // Application.sharedInstance().studyController(Application.sharedInstance().focusWindow).focusNoteInDocumentById("57F66573-E135-4591-B561-F47261585A93")
            // Application.sharedInstance().studyController(Application.sharedInstance().focusWindow).focusNoteInMindMapById("57F66573-E135-4591-B561-F47261585A93")
            // Application.sharedInstance().studyController(Application.sharedInstance().focusWindow).focusNoteInFloatMindMapById("57F66573-E135-4591-B561-F47261585A93")

            // MNUtil.studyController.focusNoteInDocumentById(focusNote.noteId)
...
try {
  ...
  search: function () {
    ...
    if (selectionText) {
      ...
      let focusNote = MNNote.getFocusNote()
      MNUtil.postNotification("searchInBrowser",{noteid:focusNote.noteId})
    }
    if (self.dynamicWindow) {
      self.hideAfterDelay()
    }
...
toolbarController.prototype.setToolbarButton = function (actionNames,newActions=undefined) {
  ...
  if (!actionNames) {
    actionNames = toolbarConfig.getDefaultActionKeys()
  ...
  for (let index = 0; index < this.buttonNumber; index++) {
    ...
    let image = actions[actionName].image+".png"
